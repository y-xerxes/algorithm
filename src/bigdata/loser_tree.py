...
def loser_tree_merge(data):
    ...
    loser_tree_length = paths_nums * 2
    loser_tree = [0] * loser_tree_length
    index_per_path = [0] * paths_nums
    offset = loser_tree_length - paths_nums

    # 败者树第一轮还需要另外记录胜者，之后就不用了
    winner = [0] * loser_tree_length
    ...
    while loser_tree[0][1] != BIG_NUM:
        ...
        print("pre_winnner: " + str(pre_winnner))
        ...
        print("new_comer: " + str(new_comer))
        loser_tree[0] = loser_tree[new_comer]
        print("loser_tree[0]: " + str(loser_tree[0]))
